...
export default {
  ...
  methods: {
    enc() {
      ...
      const pub = jsrsasign.KEYUTIL.getKey(password)
    ...
    dec() {
      ...
      const password = _this.form.password1
      const encoding = _this.form.encoding1
      const pri = jsrsasign.KEYUTIL.getKey(password)
      if (encoding === 'Base64') {
        content = jsrsasign.b64tohex(content)
      }

      const enc = jsrsasign.KJUR.crypto.Cipher.decrypt(content, pri)
      console.log('d: ' + enc)
